...
impl<Read: AsyncReadExt + Unpin + Send + Debug> RegionReader<Read> {
    pub async fn read_region_header(&mut self) -> Result<RegionHeader, Error> {
        ...
        self.read_chunk_header_to_location(&mut locations, &mut timestamps).await?;
        Ok(RegionHeader {
            locations,
            timestamps,
        })
    }
    pub async fn read_chunk_header_to_location(&mut self, locations: &mut Vec<(u32, u8)>, timestamps: &mut Vec<u32>) -> Result<(), Error> {
...
pub mod test {
    ...
    #[tokio::test]
    pub async fn test() {
        let path = Path::new("C:\\Users\\wherk\\AppData\\Roaming\\.minecraft\\saves\\New World\\region\\r.0.0.mca");
        ...
        let location = header.locations.get(2).unwrap().clone();
...
impl<Read: AsyncReadExt + AsyncSeekExt + Unpin + Send + Debug> RegionReader<Read> {
    pub async fn read_chunk_header<Location: ChunkSection>(&mut self, location: Location) -> Result<ChunkHeader, Error> {
    ...
    pub async fn read_chunk_to_bytes<Location: ChunkSection>(&mut self, location: Location) -> Result<(ChunkHeader, Vec<u8>), Error> {
        let result = self.read_chunk_header(location).await?;
        let mut data = Vec::with_capacity(result.length as usize);

        AsyncReadExt::take(&mut self.src, result.length as u64).read_to_end(&mut data).await?;
        Ok((result, data))
    }
    pub async fn read_chunk_to_value<Location: ChunkSection>(&mut self, location: Location) -> Result<(ChunkHeader, Value), Error> {
        let result = self.read_chunk_header(location).await?;
